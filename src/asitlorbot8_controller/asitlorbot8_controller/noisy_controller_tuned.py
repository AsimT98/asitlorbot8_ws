...
class NoisyController(Node):
    ...
    def __init__(self):
        ...
        self.odom_msg_.child_frame_id = "base_footprint_ekf_tuned"
    ...
    def jointCallback(self, msg):
        ...
        angular = (
            (self.wheel_radius_/(self.wheel_separation_))*(fi_left_front -fi_right_front)
            # (self.wheel_radius_/(2*self.wheel_separation_))*(fi_left_front -fi_right_front +fi_left_rear -fi_right_rear)
        )

        # Calculate the position increment
        d_s = (
            (self.wheel_radius_/2)* (dp_left_front +dp_right_front)
            # (self.wheel_radius_/4)* (dp_left_front +dp_left_rear + dp_right_rear +dp_right_front)
        )
        d_theta = (
            (self.wheel_radius_*(dp_left_front -dp_right_front))/ (self.wheel_separation_)
